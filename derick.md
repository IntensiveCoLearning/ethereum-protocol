...
可以，EIP-4844 被称为 proto-danksharding，它是以太坊即将到来的 Dencun 硬分叉（预计在 2024 年 3 月推出）中包含的一组升级提案之一，备受关注。

EIP-4844 的主要目标是通过引入一种名为「blob」的新型交易类型来改善以太坊的可扩展性。该提案并未完全实现分片，而是为即将到来的完整分片功能奠定基础。

##### EIP-4844 的工作原理：

* 「Blob」

* 这些 blob 会存储在以太坊的共识层（又称为信标链）中，但仅会保存有限的时间（通常约 18 天）。通过这种方式，EIP-4844 可以降低 rollup 将数据发布到以太坊主网的成本。
